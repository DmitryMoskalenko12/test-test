...
const CollapseItem = ({title, text, linkRef, height, setToggle,toggle, setHeight, activeTitle, setActiveTitle }) => {
    useEffect(() => {
        setHeight(linkRef.current.scrollHeight);
    }, [activeTitle]);

    return <div className={classes.content}>
            <button  onClick={(e) => {setActiveTitle(e.target.textContent); setToggle(prev => activeTitle === title ? !prev : true);}}  
            className={
            ...
            <p  style={{
                height:
                    toggle && activeTitle === title ? `${height}px` : '',
                }}
